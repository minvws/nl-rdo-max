...
def base_exception_handler(
    request: Request, redirect_uri, base_exception: RedirectBaseException
):
...
async def validation_exception_handler(
    request: Request, request_validation_error: RequestValidationError
):
    ...
    if "redirect_uri" in request.query_params:
        context["redirect_uri"] = (
            ...
            + f"?error=invalid_request&error_message={exception_message}"
        )
    return templates.TemplateResponse("exception.html", context)
...
async def general_exception_handler(request: Request, _: Exception):
    ...
    return base_exception_handler(
        ...
        redirect_uri,
        ServerErrorException(
            error_description="Server error", redirect_uri=redirect_uri
        ),
    )


async def template_base_exception_handler(
    request: Request, json_base_exception: TemplateBaseException
):
...
async def json_base_exception_handler(
    request: Request, json_base_exception: JsonBaseException
):
    redirect_uri = json_base_exception.redirect_uri
