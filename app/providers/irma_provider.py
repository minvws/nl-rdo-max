...
class IRMAProvider:
    ...
    def irma_session(
        ...
        authentication_request_state = self._authentication_cache.get_authentication_request_state(state)
        irma_response = requests.post(
            f"{self._irma_internal_server_url}/session",
            headers={"Content-Type": "application/json"},
            data=json.dumps(authentication_request_state.authentication_state["irma_context"]),
        )
        if irma_response.status_code < 400:
            irma_response_obj = irma_response.json()
            authentication_request_state.authentication_state["token"] = irma_response_obj["token"]
