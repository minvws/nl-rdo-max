...
class RateLimiter:
    ...
    def ip_limit_test(
        ...
        if ip_key_exists > nof_attempts_s:
            raise TooManyRequestsFromOrigin(ip_address)
    ...
    def user_limit_test(self, idp_prefix: str, user_limit_key: str) -> None:
        ...
        timeslot_key = f"inge6:limiter:{idp_prefix.upper()}:{str(timeslot)}"
    ...
    def rate_limit_test(self, ip_address: str) -> str:
        ...
        if primary_idp is not None:
            ...
            raise ExpectedRedisValue(
                f"Expected {self.settings.primary_idp_key} key to be set in the cache. Please check the primary_idp_key setting"
            )
