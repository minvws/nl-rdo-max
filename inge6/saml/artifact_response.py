...
class ArtifactResponse:
    ...
    def from_string(
        ...
        return cls.parse(
            settings, artifact_response_tree, provider, insecure, strict=strict
        )
    ...
    def validate_in_response_to(self) -> List[ValidationError]:
        expected_entity_id = from_settings(
            self.id_provider.settings_dict, "sp.entityId"
        )
    ...
    def validate_recipient_uri(self) -> List[ValidationError]:
        ...
        expected_response_dest = from_settings(
            self.id_provider.settings_dict, "sp.assertionConsumerService.url"
        )
    ...
    def validate_attribute_statement(self, root):
        ...
        expected_service_uuid = from_settings(
            self.id_provider.sp_metadata.settings_dict,
            "sp.attributeConsumingService.requestedAttributes.0.attributeValue.0",
        )
    ...
    def _decrypt_enc_key(self, enc_key_elem) -> bytes:
        aes_key = OneLogin_Saml2_Utils.decrypt_element(
            enc_key_elem, self.id_provider.priv_key, debug=True
        )
        return aes_key
    ...
    def get_bsn(self, authorization_by_proxy: bool) -> Text:
        if self.id_provider.saml_is_new_version:
            if "urn:nl-eid-gdi:1.0:LegalSubjectID" in self.attributes:
                self.log.info("Using LegalSubjectID from ArtifactResponse. User retrieving BSN as 'gemachtigde'")
                bsn_element = self.attributes["urn:nl-eid-gdi:1.0:LegalSubjectID"]
            else:
                if authorization_by_proxy:
                    raise ValueError("Expected LegalSubjectID in the attributes, but was not found.")
