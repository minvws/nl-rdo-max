...
from jwcrypto.jwt import JWT, JWK, JWE
...
def test_pubencrypt_data():
    ...
    assert json.dumps(data) == base64.b64decode(decrypt_box.decrypt(payload, encoder=Base64Encoder)).decode()

def _create_x25519_privkey(key):
    jwk = JWK()
    jwk._import_pyca_pri_okp(key)
    return jwk
...
def test_jwt_encryption():
    ...
    ET = JWT(key=jwk_enc_privkey, jwt=encoded_jwe)
    ST = JWT(key=jwk_sign_pubkey, jwt=ET.claims)
