...
def test_missing_setting_files(
    section, setting, mocker, mock_use_ssl
...
def test_identity_providers_example(mock_valid_id_providers_settings_path):
    ...
    get_settings().saml.identity_provider_settings = (
        mock_valid_id_providers_settings_path
    )
    assert validate_startup()
...
def test_identity_providers_example_errorneous_base_dir(
    mock_invalid_id_provider_setting_path, mocker
):
    mock_logger = mocker.patch("inge6.main.log")
    tmp = get_settings().saml.identity_provider_settings
    get_settings().saml.identity_provider_settings = (
        mock_invalid_id_provider_setting_path
    )
    assert not validate_startup()
    mock_logger.error.assert_called()
    get_settings().saml.identity_provider_settings = tmp
...
def test_identity_providers_example_errorneous(
    mock_invalid_id_provider_setting_path, mocker
):
    mock_logger = mocker.patch("inge6.main.log")
...
def test_redis_settings_errorneous(
    capture_logging, invalid_redis_settings
):  # pylint: disable=unused-argument, redefined-outer-name
    assert not validate_startup()
    keys = [
        "host",
